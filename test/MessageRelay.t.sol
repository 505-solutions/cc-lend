...
contract MessageRelayTest is Test {
    ...
    function setUp() public {
        ...
        ethPool.configureAsset(
            address(ethAsset), address(0xc89b59096964e48c6A1456c08a94D6b2A0f6Fa5B), 0.5e18, 0, ETH_FTSO_IDX, true
        );
        ethPool.setInterestRateModel(address(ethAsset), address(interestRateModel));

        flarePool.configureAsset(
            address(flareAsset), address(0x65d6a4ee7b2a807993b7014247428451aE11a471), 0.5e18, 0, TOKENX_FTSO_IDX, false
        );
        ...
        ethPool.configureAsset(
            address(ethBorrowAsset),
            address(0x013bbC069FdD066009e0701Fe9969d4dDf3c7e4E),
            0,
            1e18,
            TOKENX_FTSO_IDX,
            false
        ...
        flarePool.configureAsset(
            address(flareBorrowAsset), address(0x47d8BAC6C022CaC838f814A67e2d7A0344580D6D), 0, 1e18, ETH_FTSO_IDX, true
        );
    ...
    function testVerifyCrossChainAction() public {
        ...
        // TODO: The proof is only valid for 2 weeks so this test will fail after that.
        // TODO: Instead of hardcoding we would need to setup a mock contract.
